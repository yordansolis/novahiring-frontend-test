...
export function CandidateLoginForm() {
  ...
  useEffect(() => {
    if (hasCandidateToken()) router.replace("/interview")
  }, [router])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(true)
    try {
      const data = await loginCandidate({ username, password })
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="rounded-xl border border-[var(--ds-border)] bg-[var(--ds-background-200)] p-8 shadow-xl">
        <div className="mb-7 flex flex-col gap-1">
          <h1 className="text-xl font-semibold text-[var(--ds-gray-1000)]">
            Acceder a tu entrevista
          </h1>
          <p className="text-sm text-[var(--ds-gray-600)]">
            Introduce las credenciales que te enviamos para comenzar.
          </p>
        </div>
      ...
      <p className="text-center text-sm text-[var(--ds-gray-600)]">
        ¿Todavía no has enviado tu CV?{" "}
        <Link
          href={`/upload?job_id=${process.env.NEXT_PUBLIC_JOB_ID ?? ""}`}
