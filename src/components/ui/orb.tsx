    ...
    <div className={className ?? "relative h-full w-full"}>
      ...
      >
        <Scene
          ...
          {...(inputVolumeRef !== undefined ? { inputVolumeRef } : {})}
          {...(outputVolumeRef !== undefined ? { outputVolumeRef } : {})}
          {...(getInputVolume !== undefined ? { getInputVolume } : {})}
          {...(getOutputVolume !== undefined ? { getOutputVolume } : {})}
        />
      </Canvas>
    </div>
  )
...
function Scene({
  ...
  useEffect(() => {
    const apply = () => {
      ...
      if (uInverted) uInverted.value = isDark ? 1 : 0
    }
  ...
  useFrame((_, delta: number) => {
    ...
    if (!uTime || !uOpacity || !uAnimation || !uInputVolume || !uOutputVolume || !uColor1 || !uColor2) return

    uTime.value += delta * 0.5

    if (uOpacity.value < 1) {
      uOpacity.value = Math.min(1, uOpacity.value + delta * 2)
    }
    ...
    uAnimation.value += delta * animSpeedRef.current
    uInputVolume.value = curInRef.current
    uOutputVolume.value = curOutRef.current
    uColor1.value.lerp(targetColor1Ref.current, 0.08)
    uColor2.value.lerp(targetColor2Ref.current, 0.08)
  })
